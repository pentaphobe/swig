...
swigApi.error = function error(message, extra) {
  console.error(message);
  if (typeof extra === 'object') {
    console.error( Object.getOwnPropertyNames(extra).map( function(key) { return key + ': ' + extra[key]; } ).join('\n') );
  }
  console.error('######### FILE STACK TRACE #########');
  this.fileStack.forEach( function (entry) {
    console.log(entry);
  });
};
swigApi.fileStack = [];
swigApi.pushState = function pushState(opts) {
  this.fileStack.push(opts);
};
swigApi.popState = function popState() {
  return this.fileStack.pop();
};
swigApi.peekState = function peekState() {
  return this.fileStack.slice(-1);
};
...
exports.Swig = function (opts) {
  ...
  this.parseFile = function (pathname, options) {
    ...
    swigApi.pushState({pathname:pathname, options:options});
    ...
    var result = self.parse(src, options);
    swigApi.popState();
  ...
  this.renderFile = function (pathName, locals, cb) {
    ...
    if (this.options.debug.showFilenames) {
<<<<<<< HEAD
      return '\n<!-- START:' + pathName + 'locals:' + JSON.stringify(locals) + '-->\n' +
            self.compileFile(pathName)(locals) + '\n' + 
            '<!-- END:' + pathName + ' -->\n';
=======
      return `\n<!-- START:${pathName} locals: ${JSON.stringify(locals)} -->
${self.compileFile(pathName)(locals)}
...
>>>>>>> origin/master
  ...
  this.compileFile = function (pathname, options, cb) {
    ...
    var result = self.compile(src, options);
    swigApi.popState();
